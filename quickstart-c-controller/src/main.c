/*******************************************************************************************
*
*   quickstart-c-controller — gamepad input probe (adapted from raylib core_input_gamepad)
*
...
#define PS_ALIAS_PLAYSTATION "playstation"
#define PS_ALIAS_SONY "sony"
#define PS_ALIAS_DUALSENSE "dualsense"
#define PS_ALIAS_DUALSHOCK "dualshock"
#define PS_ALIAS_WIRELESS "wireless controller"

static bool IsPlayStationStyleName(const char *nameLower)
{
    return (TextFindIndex(nameLower, PS_ALIAS_PLAYSTATION) > -1)
        || (TextFindIndex(nameLower, PS_ALIAS_SONY) > -1)
        || (TextFindIndex(nameLower, PS_ALIAS_DUALSENSE) > -1)
        || (TextFindIndex(nameLower, PS_ALIAS_DUALSHOCK) > -1)
        || (TextFindIndex(nameLower, PS_ALIAS_WIRELESS) > -1);
}

static bool IsXboxStyleName(const char *nameLower)
{
...
int main(void)
{
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        BeginDrawing();
            ...
            {
                const char *nameLower = TextToLower(GetGamepadName(gamepad));
                ...
                else if (IsPlayStationStyleName(nameLower))
                {
